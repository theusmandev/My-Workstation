...
def scrape_novel_page(novel_url):
    ...
    try:
        ...
        # Extract Title - Checking different possible selectors
        title_tag = soup.find("h1", class_="post-title entry-title")  # Old selector
        if not title_tag:
            title_tag = soup.find("h1", class_="entry-title")  # Alternate selector
        if not title_tag:
            title_tag = soup.find("title")  # Fallback to page <title>
