...
def search_novels(query):
    ...
    results = df[df["score"] > 0.1].sort_values(by="score", ascending=False)[:50]
    return results[["title", "link"]].to_dict(orient="records")

def autocomplete(query):
    """ Fast autocomplete using binary search """
    query = query.lower()
    titles = sorted(df["normalized_title"].tolist())
...
def recommend_knn(title):
    """ Recommend novels using collaborative filtering """
    title = normalize_text(title)
    idx = df[df["normalized_title"] == title].index
    if not len(idx):
        return []
    
    idx = idx[0]
    distances, indices = knn_model.kneighbors(tfidf_matrix[idx], n_neighbors=6)
    recommendations = df.iloc[indices[0][1:]][["title", "link"]].to_dict(orient="records")
...
@app.route("/recommend_knn", methods=["GET"])
def recommend_knn_api():
    """ API endpoint for KNN recommendations """
    title = request.args.get("title", "").strip()
    if not title:
        return jsonify({"error": "Title parameter is required"}), 400
    recommendations = recommend_knn(title)
