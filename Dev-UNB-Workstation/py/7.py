...
def normalize_text(text):
    ...
    if not isinstance(text, str):
        return ""
    text = re.sub(r"[^ء-یa-zA-Z0-9\s]", "", text)  # ✅ اردو الفاظ باقی رہیں گے
    return re.sub(r"\s+", " ", text).strip().lower()

df["normalized_title"] = df["title"].apply(normalize_text)
df = df.dropna(subset=["normalized_title"])  # ✅ خالی values ہٹا دیں

def train_tfidf():
    """ Train and save TF-IDF model """
    vectorizer = TfidfVectorizer(ngram_range=(1,2), max_features=5000, stop_words=None)  # ✅ stop_words=None
